...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
	    NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"expression.plist"];
...
- (void)commonInit {
    ...
    UIColor *commonLinkColor =[UIColor colorWithRed:122.0/255 green:135.0/255 blue:54.0/255 alpha:1];
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    for (NSTextCheckingResult *result in emojis) {
		...
		if (imageName) {
			...
			CustomGlyphMetricsRef metrics = (CustomGlyphMetricsRef)malloc(sizeof(CustomGlyphMetrics));
