...
-(void)returnCount:(afterClick)block{
    if (self.returnCountBlock != block) {
        self.returnCountBlock = block;
    }
}
...
- (void)onClickPlusButton:(UIButton *)sender {
    ...
    self.currentCount += 1;
    self.countTextField.text = [NSString stringWithFormat:@"%ld", self.currentCount];
    if (self.currentCount >= self.maxCount) {
        self.plusButton.enabled = NO;
    }
    //block传值
    if (self.returnCountBlock) {
        self.returnCountBlock(self.getCurrentCount);
    }
