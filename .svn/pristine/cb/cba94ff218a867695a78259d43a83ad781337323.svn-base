...
//选中的日期
@property (nonatomic, assign) NSInteger selectYear; //!< 所选年份
@property (nonatomic, assign) NSInteger selectMonth; //!< 所选月份
@property (nonatomic, assign) NSInteger selectDay; //!< 所选天
@property (nonatomic, strong) UILabel *todayLabel; // 灰色今天 label
...
#define kCalendarBasicColor [UIColor colorWithRed:208/255.0 green:17/255.0 blue:27/255.0 alpha:1.0]
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    ...
                    //移除上一次的label 重新add到今天上
                    [_todayLabel removeFromSuperview];
                    _todayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, kPLUS_SCALE_X(2), cell.frame.size.width, 10)];
                    _todayLabel.text = @"今日";
                    _todayLabel.textAlignment = NSTextAlignmentCenter;
                    _todayLabel.font = [UIFont systemFontOfSize:8];
                    _todayLabel.textColor = [UIColor lightGrayColor];
                    [cell addSubview:_todayLabel];
                } else{
                    cell.todayCircle.backgroundColor = [UIColor clearColor];
                }
            } else {
