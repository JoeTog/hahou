...
//URLArray中可以放nsurl、uiimage
+ (UIImage *)groupIconWithURLArray:(NSArray *)URLArray bgColor:(UIColor *)bgColor;
{
    UIImageView *imageView = [[UIImageView alloc] init];
    NSMutableArray *imageArray = [NSMutableArray array];
    for (int i = 0; i<URLArray.count;  i++) {
        if ([URLArray[i] isKindOfClass:[NSURL class]]) {
            NSData * data = [[NSData alloc]initWithContentsOfURL:URLArray[i]];
            UIImage *image;
            if (data) {
                UIImageView *imageV = [UIImageView new];
                //进行缓存图片
                [imageV sd_setImageWithURL:URLArray[i] placeholderImage:[UIImage imageNamed:defaultHeadImaghe] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                }];
                //将图片直接赋值到界面 跳过了缓存
                image = [[UIImage alloc] initWithData:data];
            }else{
                UIImageView *imageV = [UIImageView new];
                [imageV sd_setImageWithURL:URLArray[i] placeholderImage:[UIImage imageNamed:defaultHeadImaghe]];
                image = imageV.image;
            }
            if (image) {
                [imageArray addObject:image];
            }else{
                [imageArray addObject:[UIImage imageNamed:@"联系人默认头像"]];
            }
        }else if ([URLArray[i] isKindOfClass:[UIImage class]]){
