...
-(void)afterClickHeadImage:(clickHeadView)block{
    if (self.clickHeadImage != block) {
        self.clickHeadImage = block;
    }
}
...
- (void)ShowHeadImageWithUrlStr: (NSString *)URLStr withUerId:(NSString *)userId completion:(ResultDown)completion
{
    ...
    self.image = nil;
    //本地游这个名字的图片
    UIImage *localImage = [UIImage imageNamed:URLStr];
    if (localImage) {
        [self setImage:localImage];
        [NFUserEntity shareInstance].mineHeadViewImage = localImage;
        return;
    }
    //图片url非法
    if (URLStr.length < 10 || ([URLStr containsString:@"null"] && URLStr.length < 30)) {
        UIImage *image = [UIImage imageNamed:defaultHeadImaghe];
        [self setImage:image];
        [NFUserEntity shareInstance].mineHeadViewImage = image;
        return;
    }else if([URLStr containsString:@"null"] && URLStr.length > 30){
