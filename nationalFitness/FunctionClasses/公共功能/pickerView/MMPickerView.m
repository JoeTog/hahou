...
-(void)initializePickerViewInView: (UIView *)view
  ...
  if (chosenObject!=nil) {
      if ([_pickerViewArray containsObject:chosenObject]) {
  ...
  [_pickerViewContainerView setBackgroundColor: [UIColor colorWithRed:0.412 green:0.412 blue:0.412 alpha:0.7]];
    ...
    _pickerViewContainerView.alpha = 0.7;
  [self addSubview:_pickerViewContainerView];
  
  //PickerView Container with top bar
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, SCREEN_WIDTH, 260.0)];
  ...
  */
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, SCREEN_WIDTH, 216.0)];
