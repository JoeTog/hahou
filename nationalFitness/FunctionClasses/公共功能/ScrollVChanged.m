...
-(instancetype)initWithCreateScrollerWithOutIndexLabelWithFrame:(CGRect)frame andChange:(NSArray *)array AndRadius:(CGFloat)num{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i=0; i<array.count; i++) {
            ...
            //默认背景颜色
            _button.backgroundColor = SsecondGray;
            if (i == 0) {
                [_button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                _button.backgroundColor = [UIColor colorMainTextColor];
                _selectedBtn = _button;
            }
            if (i==0) {
                _button.selected = YES;
            }
            [_button addTarget:self action:@selector(WithOutIndexLabelScrollerChangeClick:) forControlEvents:UIControlEventTouchDown];
            _button.tag = i+1;
            
//            ViewBorderRadius(_button, 0, 1, FirstGray);
            //设置圆角
//            if (i == 0) {
//                NFbaseViewController *baseV = [NFbaseViewController new];
//                [baseV setBtnRound:_button TopLeft:YES TopRight:NO BottomLeft:YES BottomRight:NO cornerRadii:num];
//            }
//            if (i == 1) {
//                NFbaseViewController *baseV = [NFbaseViewController new];
//                [baseV setBtnRound:_button TopLeft:NO TopRight:YES BottomLeft:NO BottomRight:YES cornerRadii:num];
//            }
...
-(void)setIsNeedLine:(BOOL)isNeedLine{
    if (isNeedLine) {
        CGFloat width = (JOESIZE.width - 2*GAP)/_array.count;
...
//
//当按钮被点击之后
-(void)WithOutIndexLabelScrollerChangeClick:(UIButton *)sender{
    [_selectedBtn setTitleColor:MainTextColor forState:(UIControlStateNormal)];
    _selectedBtn.backgroundColor = SsecondGray;
...
-(UIScrollView *)createScrollViewWithImageArr:(NSArray *)imageArray andFrame:(CGRect)frame contentSizeWidth:(CGFloat)width{
    ...
    for (int i=0; i<imageArray.count; i++) {
        ...
        [imageV sd_setImageWithURL:[NSURL URLWithString:imageArray[i]] placeholderImage:[UIImage imageNamed:defaultHeadImaghe]];
