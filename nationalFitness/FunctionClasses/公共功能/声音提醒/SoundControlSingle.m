...
//获取声音
+ (id)sharedInstanceForSound:(NSString *)soundName
{
    //sms-received1
    @synchronized ([SoundControlSingle class]) {
        _sharedInstanceForSound = [[SoundControlSingle alloc] initForPlayingSystemSoundEffectWith:soundName ofType:@"caf"];
    }
 ...
 */
//Users/joe/Desktop/聊天/nationalFitness/FunctionClasses/公共功能/声音提醒/音频资源/katalk.caf
-(id)initForPlayingSystemSoundEffectWith:(NSString *)resourceName ofType:(NSString *)type{
    self=[super init];
    if(self){
        if (!resourceName) {
//            resourceName = @"katalk2";
            resourceName = @"katalk";
        }
        //获取到系统文件中的声音
        NSString *path = [NSString stringWithFormat:@"/System/Library/Audio/UISounds/%@.%@",resourceName,type];
        path=[NSString stringWithFormat:@"%@%@%@.%@",[[NSBundle mainBundle] resourcePath],@"/",resourceName,type];
